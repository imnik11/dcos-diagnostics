...
func TestDiagnosticsJobInit(t *testing.T) {
	...
	err := job.Init()
	assert.EqualError(t, err, "could not init diagnostic job: could not initialize external log providers: open test_endpoints-config.json: no such file or directory")
